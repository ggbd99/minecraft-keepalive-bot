...
const CONFIG = {
  ...
  version: "1.21.10", // Added explicit version to match server version 1.21.10
}
...
function handleReconnect() {
  if (reconnectAttempts < MAX_RECONNECT_ATTEMPTS) {
    reconnectAttempts++
    console.log(
      `[${new Date().toLocaleTimeString()}] 🔄 Reconnecting in ${RECONNECT_DELAY / 1000} seconds... (Attempt ${reconnectAttempts}/${MAX_RECONNECT_ATTEMPTS})`,
    )
    setTimeout(() => {
      createBot()
    }, RECONNECT_DELAY)
    ...
    console.log(
      `[${new Date().toLocaleTimeString()}] ✗ Max reconnection attempts reached. Resetting counter and trying again in 30 seconds...`,
    )
    reconnectAttempts = 0
    setTimeout(() => {
      createBot()
